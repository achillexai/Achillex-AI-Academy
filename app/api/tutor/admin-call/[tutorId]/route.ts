...
// DELETE ROUTE: Handles deletion of both admin-created and user-created tutors
export async function DELETE(
  ...
  try {
    ...
    // First try to delete as admin tutor
    let [deletedTutor] = await db
      .delete(Tutor)
      .where(
        and(eq(Tutor.id, params.tutorId), eq(Tutor.userId, "admin@achillex"))
      )
      .returning();

    // If no admin tutor found, try to delete as user tutor
    if (!deletedTutor) {
      [deletedTutor] = await db
        .delete(Tutor)
